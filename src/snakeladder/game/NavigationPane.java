  ...
  implements GGButtonListener
{
  ...
  private boolean isRollsRemaining() {
	  if(rollsRemaining<1) {
		  return false;
	  }
	  return true;
  ...
  private void resetRolls() {
	  this.rollsRemaining = numberOfDice;
  }
  ...
  private LinkedList<Integer> playerIndexs;
  NavigationPane(Properties properties)
  {
    //player Index
    playerIndexs = new LinkedList<Integer>();
    ...
    this.rollsRemaining = this.numberOfDice;

    //preparing Player index
    preparePlayerIndexs();
  ...
  //prepare Indexes
  private void preparePlayerIndexs(){
    for(int i=0;i<numberOfDice;i++){
      playerIndexs.add(i);
      //System.out.println("::::"+playerIndexs.get(i));
    }
  }
  ...
  private int getDieValue() {
    ...
    currentRound = nbRolls / gp.getNumberOfPlayers();
    
    if(!isRollsRemaining())
    	playerIndex = nbRolls % gp.getNumberOfPlayers();
  ...
  */
  ...
  private int getDieValue(){
    int toRollTag=0;
    System.out.println("RM:::::::::::"+getRollRemaining());
    if(isRollsRemaining()){
      currentPlayerIndex = playerIndexs.get(0);
      try {
        toRollTag = dieValues.get(currentPlayerIndex).remove(0);
      }catch (IndexOutOfBoundsException e){
        toRollTag = RANDOM_ROLL_TAG;
        System.out.println("RRRRRRRRRRRRRRRRRRANNNNNNNNNNNNNNNNNN");
      }
    }
    return toRollTag;
  }
  ...
  private void switchToNextPlayerIndex(){
    playerIndexs.addLast(playerIndexs.removeFirst());
  }

  private boolean hasDivalues(int playerIndex){
    if(dieValues.size()<=0){return false;}
    return true;
  }
  ...
  void prepareRoll(int currentIndex)
  {
    ...
    else
    {
      ...
      showResult(result);
      if(!isRollsRemaining()) {
        System.out.println("Switchhhhhhhh player");
  ...
  void startMoving(int nb)
  {
    ...
    noOfSteps+=nb;
    if(isRollsRemaining()) {
    	showStatus("Waiting to rolls to complete...");
    	if (isAuto) {
            Monitor.wakeUp();
          } else if (gp.getPuppet().isAuto()) {
            Monitor.wakeUp();
    	...
    	showStatus("Moving...");
    	showStatus("NS:::"+noOfSteps);
        showPips("Pips: " + noOfSteps);
    	gp.getPuppet().go(noOfSteps);
        noOfSteps=0;
    }
    //showStatus("NS:::"+noOfSteps);
